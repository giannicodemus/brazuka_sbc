  ...
  <Modal :open="open" title="Adicionar Tronco VoIP" @update:open="$emit('update:open', false)">
    <ModalBody>
      <FormContainer>
        ...
        <Row>
          <Input required classInput="h-10" placeholder="Dígitos" label="Remover Dígitos" type="number" v-model="form.removedigits" />
        </Row>
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.post("/trunkvoip/store", form.value);
