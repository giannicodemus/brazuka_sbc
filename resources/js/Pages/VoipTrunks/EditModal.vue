<template>
  <Modal :open="open" title="Editar Conta VoIP" @update:open="$emit('update:open', false)">
    <ModalBody>
      <FormContainer>
        <Row>
        ...
        <Row v-if="form.auth_type === '0'">
          ...
          <Input required classInput="h-10" placeholder="Senha" label="Senha" type="text" v-model="form.password" />
        </Row>
        <Row v-if="form.auth_type === '1'">
          <Input required classInput="h-10" placeholder="192.168.0.1" label="Host" v-model="form.host" />
          <Input required classInput="h-10" placeholder="Techprefix" label="Techprefix" type="number"
            v-model="form.techprefix" />
        </Row>
        <Row>
          <Input required classInput="h-10" placeholder="Dígitos" label="Remover Dígitos" type="number"
            v-model="form.removedigits" />
        </Row>
...
watch(() => props.selected, (selectActions) => {
  if (selectActions) {
    form.value = {
      id: selectActions.id || "",
      name: selectActions.name || "",
      codec: selectActions.codec || "",
      auth_type: String(selectActions.auth_type ?? "0"),
      username: selectActions.username || "",
      password: selectActions.password || "",
      host: selectActions.host || "",
      techprefix: selectActions.techprefix || "",
      removedigits: String(selectActions.removedigits ?? "0"),
    };
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.put(`/trunkvoip/update/${form.value.id}`, {
      ...form.value,
    });
