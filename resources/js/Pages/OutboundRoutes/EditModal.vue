...
watch(() => props.selected, (selectActions) => {
  if (selectActions) {
    form.value = {
      id: selectActions.id || "",
      voip_account_id: selectActions.voip_account_id || "",
      voip_trunk_id: selectActions.voip_trunk_id || "",
      random_clid_list_id: selectActions.random_clid_list_id || "",
      recording: String(selectActions.recording ?? "0"),
    };
  }
...
const submitForm = async () => {
  ...
  try {
    const response = await axios.put(`/outboundroutes/update/${form.value.id}`, {
      ...form.value,
    });
